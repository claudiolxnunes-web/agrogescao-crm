 ...
 * Importa Clientes_Ativos.xlsm e Clientes_Inativos.xlsm para o banco MySQL
 *
 * Regras:
 ...
 * - Ativos têm representante quando disponível
 * - Upsert pelo código do cliente
 *
 * Uso: node import_clientes_ativo_inativo.mjs [caminho_ativos] [caminho_inativos]
 */
...
const ATIVOS_PATH   = args[0] || "./Clientes_Ativos.xlsm";
const INATIVOS_PATH = args[1] || "./Clientes_Inativos.xlsm";
...
function parseRepCode(erc) {
  if (!erc) return null;
  const m = String(erc).match(/^(\d{6})/);
  return m ? m[1] : null;
}
...
function isInativo(ultimaCompra) {
  if (!ultimaCompra) return true; // sem data = considera inativo
...
function readClientes(filePath, fileIsAtivo) {
  ...
  for (const row of dataRows) {
    ...
    // Representante: APENAS para ativos e quando a coluna ERC existir
    const ercRaw = fileIsAtivo ? null : get(row, "ERC");
    // Ativos não têm coluna ERC no arquivo — repCode sempre null aqui
    // (será vinculado pelo banco se o clientCode já existir com rep)
    const repCode = null; // inativos: sem rep; ativos: sem coluna ERC → sem rep por enquanto
    ...
    // Tipo: prioridade revenda > fábrica > fazenda
  ...
  const result = Array.from(map.values()).map(e => ({
    ...
    // Status: inativo se arquivo de inativos OU se última compra > 6 meses
...
async function main() {
  ...
  // Merge: ativos têm prioridade (sobrescrevem inativos para o mesmo código)
  ...
  // Busca representantes para vinculação (só para ativos que tiverem repCode)
  const [repsRows] = await conn.execute(
    "SELECT id, code FROM representatives WHERE code IS NOT NULL"
  );
  const repMap = new Map(repsRows.map(r => [r.code, r.id]));

  let inserted = 0, updated = 0, errors = 0;

  for (const c of all) {
    try {
      // Inativos nunca têm representante vinculado
      const repId = (c.status === "active" && c.repCode)
        ? (repMap.get(c.repCode) ?? null)
        : null;
      ...
      if (existing.length > 0) {
        // Mantém representante existente se novo valor for null
        const keepRepId = repId ?? existing[0].representativeId ?? null;
