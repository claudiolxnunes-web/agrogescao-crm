...
export default function Analytics() {
  ...
  const { data: forecast } = trpc.analytics.forecast.useQuery({ regionId });
  const { data: funnel } = trpc.analytics.conversionFunnel.useQuery({ regionId });
  const { data: heatmap } = trpc.analytics.activityHeatmap.useQuery();
  ...
  const funnelData = (funnel || [])
    .filter(f => f.stage !== null && !["won", "lost"].includes(f.stage!))
    .map((f, i) => ({
