  ...
  importFaturamento: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          const clientCode = toStr(row["Cód. Cliente"]);
          const clientName = toStr(row["Nome do Cliente"]);
          const repCode = toStr(row["Cód. RC"]);
          const repName = toStr(row["Representante"]);
          const productCode = toStr(row["Cód. Produto"]);
          const productName = toStr(row["Nome do Produto"]);
          const invoiceNumber = toStr(row["Nota Fiscal"]);
          const orderNumber = toStr(row["Pedido"]);
          ...
          if (isOpenOrder(row)) {
            ...
            if (existingInvoiceKeys.has(orderKey)) {
              ...
              await db.insert(openOrders).values({
                ...
                faturamentoEstimado: toNum(row["Faturamento Realizado"]),
                volumeEstimado: toNum(row["Volume (Vendas)"]),
