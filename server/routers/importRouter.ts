...
- O campo "name" (Nome) é obrigatório para clientes`;
  ...
  importClients: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Pré-carregar registros existentes para evitar duplicatas
      const existingClientCodes = new Set<string>();
      const existingClientNames = new Set<string>();
      const allExistingClients = await db.select({ clientCode: clients.clientCode, name: clients.name }).from(clients);
      for (const ec of allExistingClients) {
        if (ec.clientCode) existingClientCodes.add(ec.clientCode.trim());
        if (ec.name) existingClientNames.add(ec.name.trim().toLowerCase());
      }
      ...
      for (let batchStart = 0; batchStart < dataRows.length; batchStart += BATCH_SIZE) {
        ...
        for (let i = 0; i < batch.length; i++) {
          ...
          // Verificar duplicata por clientCode (se mapeado) ou por nome
          const rowClientCode = getValue(row, "clientCode")?.trim();
          if (rowClientCode && existingClientCodes.has(rowClientCode)) { skipped++; continue; }
          if (!rowClientCode && existingClientNames.has(name.trim().toLowerCase())) { skipped++; continue; }
